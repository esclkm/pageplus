...
{
	$ppf_fields = $ppf_fields + $cot_extrafields[$db_pages];
}
...
$ppf_pages_cat_list['all'] = $L['All'];
$ppfcatlist = cot_structure_children('page', $c);

foreach ($ppfcatlist as $ppfcat)
{
	$ppf_pages_cat_list[$ppfcat] = $structure['page'][$ppfcat]['title'];
}

function cot_ppf_selectcat($cat)
{
	global $structure, $sys, $cfg, $db;
	$ppf_pages_cat_list['all'] = $L['All'];
	$ppfcatlist = cot_structure_children('page', $cat);

	foreach ($ppfcatlist as $ppfcat)
	{
		$ppf_pages_cat_list[$ppfcat] = $structure['page'][$ppfcat]['title'];
	}
	return cot_selectbox((!isset($ppf['ppfcat'])) ? 'all' : $ppf['ppfcat'], 'ppf[ppfcat][]', array_keys($ppf_pages_cat_list), array_values($ppf_pages_cat_list), false, ' multiple="multiple" style="width:50%"');
}
